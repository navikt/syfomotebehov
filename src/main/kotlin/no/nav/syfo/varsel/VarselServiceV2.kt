...
class VarselServiceV2 @Inject constructor(
    ...
    fun sendSvarBehovVarsel(ansattFnr: String, kandidatUuid: String) {
        ...
        log.info("Antall unike nærmeste ledere for kandidatUuid $kandidatUuid: ${narmesteLederRelations.size}, antall virksomheter: $amountOfVirksomheter")
    ...
    private fun shouldSendVarselTilArbeidstaker(ansattFnr: String, oppfolgingstilfelle: PersonOppfolgingstilfelle?): Boolean {
    ...
    private fun narmesteLeder(ansattFnr: String, virksomhetsnummer: String): NarmesteLederRelasjonDTO? {
        return narmesteLedere(ansattFnr).firstOrNull { virksomhetsnummer == it.virksomhetsnummer }
    }

    private fun narmesteLedere(ansattFnr: String): List<NarmesteLederRelasjonDTO> {
        return narmesteLederService.getAllNarmesteLederRelations(ansattFnr) ?: emptyList()
    }
