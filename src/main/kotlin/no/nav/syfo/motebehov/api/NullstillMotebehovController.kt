...
class NullstillMotebehovController(
    private val motebehovDAO: MotebehovDAO,
) {
    @RequestMapping(value = ["/nullstill/{aktoerId}"], produces = [MediaType.APPLICATION_JSON_VALUE])
    fun slettMotebehov(
        @PathVariable aktoerId: String,
        @Value("\${nais.cluster.name:ukjent}") env: String,
    ): String =
        if (Toggle.enableNullstill || Stream.of("dev-fss", "local").anyMatch { anObject: String -> env.equals(anObject) }) {
